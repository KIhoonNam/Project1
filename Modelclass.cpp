...
bool ModelClass::Initialize(ID3D11Device* device,  char* modelFilename,WCHAR* textureFilename1,WCHAR* textureFilename2)
{
    ...
    LoadTextureArray(device, textureFilename1,textureFilename2);
    // 이 모델의 텍스처를 로드합니다.
...
bool ModelClass::LoadTexture(ID3D11Device* device, WCHAR* filename1)
{
    ...
    // 텍스처 오브젝트를 초기화한다.
    return m_Texture->Initialize(device, filename1);
}
...
bool ModelClass::LoadTextureArray(ID3D11Device* device, WCHAR* filename1, WCHAR* filename2)
{
    // 텍스처 오브젝트를 생성한다.
